...
// ...Irrelevant code for customizing the buttons and title
        ...
        dialogView = inflater.inflate(R.layout.masterproductospedidos_layout, null);
 ...
 *//*               SimpleCursorAdapter adapter = new SimpleCursorAdapter(getApplicationContext(), android.R.layout.simple_list_item_multiple_choice, R.layout.list_item, new String[]{"Codigo", "Nombre", "Precio", "Unidad", "UnidadCaja", "Existencia"}, new int[]{R.id.Codigo, R.id.Nombre,
                        R.id.Precio, R.id.Presentacion, R.id.UnidadCaja, R.id.Existencias});*//*
                ListAdapter adapter = new  SimpleAdapter(
                        getApplicationContext(),listaArticulosItem,
                        R.layout.list_item_pedido, new String[]{"Codigo","checkbox", "Nombre", "Precio"}, new int[]{R.id.Codigo,R.id.checkbox, R.id.Nombre,
                        R.id.Precio});
        ...
        btnOK2.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                int vcantSeleccionado=0;
                String CodigoArticulo="";
                int count = lvItem.getAdapter().getCount();
                for (int i = 0; i < count; i++)
                {
                    ViewGroup row = (ViewGroup) lvItem.getChildAt(i);
                    CheckBox vCheck = (CheckBox) row.findViewById(R.id.checkbox);

                    if (vCheck.isChecked())
                    {
                        vcantSeleccionado +=1;
                        CodigoArticulo = ((TextView) row.findViewById(R.id.Codigo)).getText().toString();
                    }
                }
                if (vcantSeleccionado==1){
                    txtCodigoArticulo.setText("");
                    lblDescripcionArticulo.setText("");
                    articulo = ArticulosH.BuscarArticulo(CodigoArticulo);
